...
const MacroCalculator = () => {
  ...
  const MacroRecommendationsPanel = ({ gender, goal }) => {
    const getMacroRecommendation = (gender, goal) => {
      ...
      return recommendations[gender]?.[goal] || { protein: "30", carbs: "40", fat: "30" };
    };

  return (
    <div className="bg-white rounded-3xl shadow-lg p-8 h-fit sticky top-8">
      <h2 className="text-2xl font-bold text-black text-center mb-6">Recommended Split</h2>
      <div>
        {gender && goal && (
          <div className="bg-purple-50 p-4 rounded-lg border-l-4 border-purple-500">
            <p className="font-bold text-purple-800 text-lg mb-4">
              {gender === 'male' ? '👨 ' : '👩 '}
              Recommended for {gender === 'male' ? 'Men' : 'Women'} - {goal.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}:
            </p>
        ...
        <div className="mt-4 text-sm text-gray-600">
          <p className="mb-2">These recommendations are based on established nutritional guidelines for different fitness goals.</p>
          <p>Adjust the percentages within the recommended ranges based on your individual needs and preferences.</p>
        </div>
      </div>
